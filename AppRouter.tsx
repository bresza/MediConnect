...
export function AppRouter({ darkMode, onToggleDark }: AppRouterProps) {
  ...
  // Deriva lastVisit a partir dos agendamentos concluídos, caso o banco não tenha esse campo preenchido
  const patientsWithLastVisit = useMemo(() => {
    const lastVisitByPatient = new Map<string, string>()
    appointments.forEach((a) => {
      if (a.status === "completed" && a.patientId && a.date) {
        const current = lastVisitByPatient.get(a.patientId)
        if (!current || a.date > current) {
          lastVisitByPatient.set(a.patientId, a.date)
        }
      }
    })
    return visiblePatients.map((p) => {
      const derivedDate = lastVisitByPatient.get(p.id)
      if (!derivedDate) return p
      // Usa a data mais recente entre a do banco e a derivada
      if (!p.lastVisit || derivedDate > p.lastVisit) {
        return { ...p, lastVisit: derivedDate }
      }
      return p
    })
  }, [visiblePatients, appointments])
  ...
  // Ao marcar consulta como concluída, atualiza o lastVisit do paciente
  async function handleUpdateAppointment(a: Appointment) {
    await updateAppointment(a)
    if (a.status === "completed" && a.patientId) {
      const patient = patients.find((p) => p.id === a.patientId)
      if (patient) {
        const visitDate = a.date ?? new Date().toISOString().slice(0, 10)
        // Só atualiza se a data da consulta for mais recente que a última registrada
        if (!patient.lastVisit || visitDate >= patient.lastVisit) {
          await updatePatient({ ...patient, lastVisit: visitDate })
        }
      }
    }
  }
  ...
  function renderPage() {
    ...
    switch (activePage) {
      ...
      // ── Relatórios / Laudos ──────────────────────────────────────
      case "reports":
        return <Reports currentUser={currentUser} patients={patientsWithLastVisit} />
