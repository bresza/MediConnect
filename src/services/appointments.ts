...
async function patchAppointmentFields(
  ...
  const updated = await apiRequest<Array<Pick<ApiAppointment, "id">>>(`/rest/v1/appointments?${filter}&select=id`, {
    method: "PATCH",
  ...
  if (!Array.isArray(updated) || updated.length === 0) {
    throw new Error("Não foi possível atualizar esta consulta. Atualize a página e tente novamente.")
  }
}
